    ...
    public partial class frmSendMail : Form
    {
        ...
        public void SendAttachmentEmail()
        {
            try
            {
               ...
               // SmtpMail.SmtpServer = "plus.smtp.mail.yahoo.com";//smtp is :smtp.yahoo.com
                ...
                isSend = true;
                isCheck = true;
                MessageBox.Show("Mail sent successfully ", "Oneaccount", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception)
            {
                Cursor.Current = Cursors.Default;
                isSend = true;
                MessageBox.Show("Mail sending failed", "Oneaccount", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        public void SendMail()
        {
            try
            {
                if (ValidateEmail())
                {
                    ...
                    {
                        if (MessageBox.Show("Send this message without a subject ?", "Oneaccount", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                        {
                            if (txtBody.Text == string.Empty)
                            {
                                if (MessageBox.Show("Send this message without text in the body ?", "Oneaccount", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                                {
                    ...
                    else
                    {
                        if (txtBody.Text == string.Empty)
                        {
                            if (MessageBox.Show("Send this message without text in the body ?", "Oneaccount", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                bwrk1.RunWorkerAsync();
                                frmLodingObj.ShowFromSendMail();
                            }
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Enter your Email id to send mail", "Oneaccount", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                    txtMailId.Focus();
                }
                else
                {
