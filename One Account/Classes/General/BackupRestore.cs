...
using One_Account.Classes.General;
...
namespace One_Account
{
    class BackupRestore : DBConnection
    {
        ...
        public void TakeBackUp()
        {
            try
            {
                if (MessageBox.Show("Do you want to take back up?", "Oneaccount", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (sqlcon.State == ConnectionState.Closed)
                    {
                        sqlcon.Open();
                    }
                    string strPath = (ConfigurationManager.AppSettings["ApplicationPath"] == null || ConfigurationManager.AppSettings["ApplicationPath"].ToString() == null) ? null : ConfigurationManager.AppSettings["ApplicationPath"].ToString() + "\\Data\\" + PublicVariables._decCurrentCompanyId + "\\DBOneaccount.mdf";
                    SaveFileDialog saveBackupDialog = new SaveFileDialog();
                    string strDestinationPath = string.Empty;
                    string strFname = "DBOneaccount" + DateTime.Now.ToString("ddMMyyyhhmmss");
                    ...
                    if (saveBackupDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        MessageBox.Show("The backup of database  completed successfully", "Oneaccount", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("BR 1 : " + ex.Message, "Oneaccount", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        /// <summary>
        ...
        public void ReStoreDB()
        {
            ...
            try
            {
                ...
                opDialog.Title = "Select your .bak file for restore the database Oneaccount";
                string path = "";
                ...
                if (opDialog.ShowDialog() == DialogResult.OK)
                {
                    string filePath = Path.GetFullPath(opDialog.FileName);
                    {
                        ...
                        string strSqlRestore = "RESTORE DATABASE [" + sqlcon.Database + "] FROM DISK = '" + fileName + "' WITH MOVE '" + "DBOneaccount" + "' TO '" + dataFileLocation + "', MOVE '" + "DBOneaccount_log" + "' TO '" + logFileLocation + "'";
