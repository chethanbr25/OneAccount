            ...
            try
            {
                ...
                else
                {
                    if (RemoveIncompleteRowsFromGrid())
                    {
                        ...
                        {
                            MessageBox.Show("Can't save material receipt without atleast one product with complete details", "Oneaccount", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            dgvProduct.ClearSelection();
                            dgvProduct.Focus();
                        }
                        else
                        {
                            ...
                            if (btnSave.Text == "Update")
                            {
                                if (QuantityCheckWithReference() == 1)
                                {
                                    if (dgvProduct.Rows[0].Cells["dgvtxtProductName"].Value == null)
                                    {
                                        MessageBox.Show("Can't Edit MateraialReceipt without atleast one product with complete details", "Oneaccount", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        dgvProduct.ClearSelection();
                                        dgvProduct.Focus();
                                    }
                                    else if (decEdit == 1)
                                    {
                                        MessageBox.Show("Can't Edit MateraialReceipt with Invalid Quantity", "Oneaccount", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        dgvProduct.ClearSelection();
                                        dgvProduct.Focus();
                                    }
                                    else
